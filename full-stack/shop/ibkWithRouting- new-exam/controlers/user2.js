...
const greetingsSchema = new mongoose.Schema(
  { blessing: String, image: String }  
);

const greetingsCollection = mongoose.model("greetingsSchema", greetingsSchema);
//*********** */
...
exports.addBlessing = async (req, res, next) => {
  try {
    ...
    if (typeof data.blessing !== "string") throw new Error("blessing is not a string");

    const nameFlag = await greetingsCollection.findOne({ blessing: data.blessing, image: data.image });
    console.log('user:', nameFlag)
    // if (user !== null) {
    //     res.send({ ok: false, message: 'user with such user name already exists' })
    // } else {

    if ((data.blessing.length > 0 && data.image.length > 0) && (!nameFlag)) {

      let newRegister = new greetingsCollection({ blessing: data.blessing, image: data.image});
      newRegister.save().then(() => console.log("blessing saved"));
      res.send({ ok: true });

    } else {
      throw new Error("blessing is empty string");
    }
...
exports.searchBlessing = async (req, res, next) => {
  console.log("dddddddddddddddddddddddddddddddddddddddd")

  let co = await greetingsCollection.count();
  console.log(co);
  let first = Math.floor(Math.random() * co);
  console.log(first)
  //let x = greetingsCollection.aggregate([{$sample:{size:1}}]);
  //console.log(x._model);
  let bookFlag = [];
  bookFlag = await greetingsCollection.find({});
  console.log(bookFlag);
  console.log(bookFlag[first]);
  res.send({ ok: true, data: {blessing : bookFlag[first].blessing, image : bookFlag[first].image}})
